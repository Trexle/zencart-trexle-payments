<?php
/**
 * trexle.php payment module class for Trexle Payments method
 *
 ...
 */
include_once((IS_ADMIN_FLAG === true ? DIR_FS_CATALOG_MODULES : DIR_WS_MODULES) . 'payment/trexle/trexle_functions.php');

/**
 * trexle.php payment module class
 *
 */
class trexle extends base {
    /**
    ...
    function javascript_validation() {
        ...
        $js = '  if (payment_value == "' . $this->code . '") {   ' . "\n" .
            'if (document.checkout_payment.trexle_cc_token == undefined ||
                 (document.checkout_payment.trexle_cc_token != undefined && document.checkout_payment.trexle_cc_token.value=="update_new_token"))
                 { '.
    ...
    function selection() {
        ...
        if ($this->gateway_mode == 'offsite') {
            ...
            if ( count(zen_get_customer_trexle_token($_SESSION['customer_id'])) >0 ) {
                $selection['fields'][] =
                    array(
                ...
                $tokens = zen_get_customer_trexle_token($_SESSION['customer_id']);
                for ($ind = 0; $ind < count($tokens); $ind++) {
                    $selection['fields'][] = array('title' => zen_draw_radio_field('trexle_cc_token', $tokens[$ind]["token"], ' autocomplete="off"')." ".$tokens[$ind]["cardinfo"]." ".'<a href="javascript:delTrexleTokens(\'' . $tokens[$ind]["unique_card"] . '\')">' . zen_image(DIR_WS_IMAGES.'/'.'delete_16x16.gif') . '</a>',
                        'field' => "",
...
//                        'tag' => $this->code . '-cc-cvv-' . $ind);
                ...
                $js .= '    function delTrexleTokens(unique_card){' . "\n";
                $js .= '    var r = confirm("Are you sure you want to delete this token?");' . "\n";
                $js .= '    if (r == true) {$.ajax({'. "\n";
                $js .= '    method: "POST",'. "\n";
                $js .= '    url: "ajax.php",'. "\n";
                $js .= '    data: { delTrexleTokenAct: "del", token: unique_card },'. "\n";
                ...
                $selection['fields'][] = array(
                    ...
                    'field' => zen_draw_radio_field('trexle_cc_token', "update_new_token", false),
                    'tag' => $this->code . '-use-token');
    ...
    function pre_confirmation_check() {
        ...
        if (isset($_POST['trexle_cc_token']) && trim($_POST['trexle_cc_token']) !="update_new_token")
        {
            $this->cc_card_type = "";
            $this->cc_card_number = "";
        ...
        else
        {
            ...
            $result = $cc_validation->validate($_POST['trexle_cc_number'], $_POST['trexle_cc_expires_month'], $_POST['trexle_cc_expires_year']);
            $error = '';
            switch ($result) {
                case -1:
    ...
    function process_button() {
        if (isset($_POST['trexle_cc_token']) && trim($_POST['trexle_cc_token'])!='' && trim($_POST['trexle_cc_token']) !="update_new_token")
        {
            $process_button_string = zen_draw_hidden_field('cc_token', $_POST['trexle_cc_token']);
            ...
            $process_button_string = zen_draw_hidden_field('cc_owner', $_POST['trexle_cc_owner']) .
    ...
    function after_process() {
        ...
        $sql = $db->bindVars($sql, ':orderComments', 'Trexle payments.  AUTH: ' . $this->auth_code . '. TransID: ' . $this->transaction_id . '.', 'string');
  ...
  PRIMARY KEY (`id`)
        ...
        $db->Execute("CREATE TABLE IF NOT EXISTS `trexle_order_token` (
  `trexle_order_token` int(11) NOT NULL AUTO_INCREMENT,
  `order_id` int(11) NOT NULL,
  `token` varchar(64) NOT NULL,
  `created_date` date NOT NULL,
  PRIMARY KEY (`trexle_order_token`)
) AUTO_INCREMENT=1");
        $db->Execute("CREATE TABLE IF NOT EXISTS `trexle_tokens` (
  ...
  KEY `customer_id` (`customer_id`)
    ...
    function _debugActions($submit_data, $response, $order_time= '', $sessID = '', $new_order_id) {
        ...
        $errorMessage = date('M-d-Y h:i:s') .
            "\n=================================\n\n";
        $errorMessage .=
            'Sent to Trexle Payments: ' . print_r($this->reportable_submit_data, true) . "\n\n";
        // DATABASE SECTION
...
//        }
    ...
    private function _sendRequest($submit_data)
    {
        ...
        include_once(DIR_WS_MODULES . "payment/trexle/autoload.php");
        include_once(DIR_WS_MODULES . "payment/trexle/omnipay/trexle/src/Gateway.php");
    ...
    private function _sendRefundRequest($submit_data)
    {
        ...
        include_once(DIR_FS_CATALOG_MODULES . "payment/trexle/autoload.php");
        include_once(DIR_FS_CATALOG_MODULES . "payment/trexle/omnipay/trexle/src/Gateway.php");
...
//        include_once(DIR_WS_MODULES . "payment/trexle/autoload.php");
//        include_once(DIR_WS_MODULES . "payment/trexle/omnipay/stripe/src/Gateway.php");
//
...
//        ));
        ...
        try {
            if (zen_check_existing_customer_trexle_token($customer_id, $token) !==false)
            {

                //$stripe->deleteCustomer();
                zen_del_customer_trexle_token($customer_id, $token);
