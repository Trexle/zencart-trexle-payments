<?php
function zen_get_customer_trexle_token($customer_id){
    global $db;
    ...
    if ($customer_tokens->RecordCount() > 0) {
        while (!$customer_tokens->EOF) {
            $tokens[] = array("token"=>$customer_tokens->fields['token'],
                ...
                'trexleid'=> $customer_tokens->fields['id'],
...
function zen_check_existing_customer_trexle_token($customer_id, $unique_card){
    global $db;
